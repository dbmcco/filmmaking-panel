ABOUTME: Codex CLI instructions for Filmmaking Panel.
ABOUTME: Enforces session bootstrap and resources-first intake.

# Codex CLI Instructions (Filmmaking Panel)

## Required Startup

Before any session work, run:
```bash
...
- Follow `prompts/moderator.md` for Phase 0 elicitation and filmmaking workflow.
...
printf "%s" "$PROMPT" | codex-subagent start \
  ...
  --label "gilroy-phase-2" \
  --prompt -

codex-subagent wait --labels "gilroy-phase-2" --follow-last
```
